	...
	def train(self, x_train, y_train, x_valid, y_valid,
		...
		for epoch in range(1, max_epoch+1):
			...
			if validation and x_valid is not None and y_valid is not None:
				...
				num_samples_validation = x_valid.shape[0]
				validation_batch_size = num_samples_validation // self.conf.batch_size + 1

				for index in tqdm(range(validation_batch_size)):
					# Current batches
					x_valid_batch = x_valid[self.conf.batch_size*index:self.conf.batch_size*(index+1)]
					y_valid_batch = y_valid[self.conf.batch_size*index:self.conf.batch_size*(index+1)]

					# Run
					feed_dict = {self.inputs: x_valid_batch,
								self.labels: y_valid_batch}
					predictions = self.sess.run(self.preds,
								feed_dict=feed_dict)
					if index==0:
				...
				print('Validation accuracy for Epoch {:d}: {:.4f}'.format(
									epoch, np.sum(preds==y_valid)/y_valid.shape[0]))
				### END CODE HERE
